...
export function StationSelector({ 
  ...
  useEffect(() => {
    const handleOutsideClick = (event: MouseEvent) => {
      ...
      // Don't close if clicking on the Calculate Fare button
      if (target.closest('button')?.textContent?.includes('Calculate') ||
          target.closest('button')?.textContent?.includes('गणना')) {
        return;
      }
