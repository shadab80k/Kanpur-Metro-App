...
import { calculateFare, calculateDistanceBetweenStations } from "@/data/metroData";
import { Station } from "@/types/metro";
import { ArrowRight, Calculator, Ticket, RefreshCw } from "lucide-react";
...
const FareCalculator = () => {
  ...
  const handleCalculateFare = () => {
    ...
    // Calculate the accurate distance between the two stations
    const distance = calculateDistanceBetweenStations(selectedSource.id, selectedDestination.id);
    
    // Get the fare based on the distance
    const { fare, smartCardFare } = calculateFare(distance);
    
    // Set the fare result with smooth transition (same timing as journey planner)
    setTimeout(() => {
      setFareResult({
        fare,
        smartCardFare,
